...
class Penjualan extends CI_Controller
{
    ...
    public function keranjang()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/kasir_sidebar', $data);
            $this->load->view('templates/kasir_topbar', $data);
            $this->load->view('kasir/v_penjualan_keranjang', $data);
            $this->load->view('templates/footer', $data);
        } else {
            ...
            if ($input['bayar'] >= $data['total_harga']) {
                ...
                if ($pemesanan != '0') {
                    ...
                    redirect('Kasir/Penjualan/detail/' . $id_transaksi);
                } else {
                    set_pesan('Anda belum memasukkan barang yang akan di-checkout!', false);
                    redirect('Kasir/Penjualan/keranjang');
                }
            } else {
    ...
    public function add_item()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/kasir_sidebar', $data);
            $this->load->view('templates/kasir_topbar', $data);
            $this->load->view('kasir/v_penjualan_addItem', $data);
            $this->load->view('templates/footer', $data);
        } else {
            $input = $this->input->post(null, true);
            $input['id_item'] = time();
            ...
            if ($stok >= $input['qty']) {
                ...
                redirect('Kasir/Penjualan/add_item');
            }
        }
    }
    ...
    public function cetak_nota($id)
    {
        ...
        $data['title'] = "Penjualan";
        $this->load->view('kasir/v_cetak_nota', $data);
    }
}
